...
# Set up working directory
mkdir ../finetune-lstm
mv -t ../finetune-lstm common_voice kenlm.scorer alphabet.txt pretrained
cd ../finetune-lstm
...
# --- German: freeze 5 layers ---
# (still inside finetune-lstm working dir with ds-finetune conda env activated)
# Set up working directory
mkdir ../finetune-5layer
mv -t ../finetune-5layer common_voice kenlm.scorer alphabet.txt pretrained
cd ../finetune-5layer

# Clone and install deepspeech-transfer and install pip dependencies
git clone https://github.com/onnoeberhard/deepspeech-transfer
cd deepspeech-transfer
git checkout transfer-5
pip install -e .
cd ..
...
# Set up working directory
mkdir -p ../freeze-lstm
mv -t ../freeze-lstm pretrained alphabet.txt kenlm.scorer swiss_data deepspeech-transfer
cd ../freeze-lstm

# Install deepspeech-transfer
cd deepspeech-transfer
git checkout transfer-lstm
pip install -e .
cd ..
...
# --- Swiss: freeze 5 layers ---
# (still inside swiss/freeze-lstm working dir with ds-finetune conda env activated)
# Set up working directory
mkdir -p ../freeze5
mv -t ../freeze5 pretrained alphabet.txt kenlm.scorer swiss_data deepspeech-transfer
cd ../freeze5
